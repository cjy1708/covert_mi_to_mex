...
void redr_embed(double** x, int dim, int edim, int tau, int N, int K,
    ...
    int BOX;
    int** box, * lis; // two dimensional boxes二维长方体
    int*** boxxx; // twodimensional boxes for each dim每个尺寸的二维框
    int** lisxx; // lists for each dim每个dim的列表
    double epsilon;
    int inveps;
    ...
    //  if (edim<2) Just call usual redundancy ; if（edim<2）只调用常规冗余
...
void mi_xnyn(double** x, int dimx, int dimy, int N, int K,
    ...
    int BOX, BOX1;
    int** box, ** boxy = NULL, ** boxx = NULL, * lis; // two dimensional boxes二维长方体
    int* lisy = NULL, * lisx = NULL; // lists for two dimensions二维列表
    int* boxx1 = NULL, * boxy1 = NULL; // onedimensional boxes 一维长方体
    ...
    if (dimy > 1) {
        ...
        for (d = 0; d < dimy; d++) {
            memcpy(yy[d], x[d + dimx], N * sizeof(double));
        }
    }
    else {
